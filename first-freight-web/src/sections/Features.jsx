  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center mb-16">
      ...
      <div className={flip ? 'md:order-2' : 'md:order-1'}>
        <span className="ff-eyebrow flex items-center gap-2" style={{ color: 'var(--ff-orange)' }}>
          <span style={{ width: 24, height: 3, background: 'var(--ff-red)', display: 'inline-block' }} />
          {eyebrow}
        </span>
        <h3 style={{
          fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 'clamp(1.6rem,2.8vw,2.2rem)',
          lineHeight: 1.1, color: '#fff', margin: '14px 0 12px'
        }}>{title}</h3>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 15.5, lineHeight: 1.65, color: 'rgba(255,255,255,.68)', margin: '0 0 18px', maxWidth: 440 }}>{body}</p>
        <div style={{ display: 'grid', gap: 9 }}>
          {points.map(p => (
            <div key={p} style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'var(--font-body)', fontSize: 14.5, color: 'rgba(255,255,255,.85)' }}>
              <Check size={18} style={{ color: 'var(--ff-red)', flexShrink: 0 }} />{p}
            </div>
          ))}
        </div>
      </div>
