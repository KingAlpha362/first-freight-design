...
function GTile({ img, c, r, label, className = '' }) {
  ...
  return (
    <div
      ...
      className={className}
      style={{
        gridColumn: c, gridRow: r, position: 'relative', overflow: 'hidden',
    ...
    >
      ...
      <div style={{
        ...
        background: h ? 'linear-gradient(0deg, rgba(184,52,31,.55), transparent 60%)' : 'linear-gradient(0deg, rgba(14,12,11,.55), transparent 55%)',
  ...
  return (
    <section id="gallery" style={{ background: '#FFFFFF', color: 'var(--fg-1)', padding: '100px 30px' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        ...
        {/* Desktop bento grid */}
        <div className="hidden md:grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', gridTemplateRows: 'repeat(3, 196px)', gap: 16 }}>
        ...
        {/* Mobile simple grid */}
        <div className="grid md:hidden grid-cols-2 gap-3" style={{ gridAutoRows: 180 }}>
          {[
            ['/assets/big-team.jpg', 'The crew'],
            ['/assets/delivering.jpg', null],
            ['/assets/warehouse.jpg', 'Depot'],
            ['/assets/happy-team.jpg', 'On the road'],
            ['/assets/fleet-truck.jpg', 'National fleet'],
            ['/assets/depot-packing.jpg', null],
          ].map(([img, label], i) => (
            <GTile key={i} img={img} label={label} />
          ))}
        </div>
      </div>
    </section>
